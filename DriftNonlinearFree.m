function [objective, constraints, variables] = DriftNonlinearFree(p)
...
constraints = [ constraints
    (   diff(xE.variable)         == (- Ux.physical(1:N).*sin(Psi.physical(1:N)) - Uy.physical(1:N).*cos(Psi.physical(1:N))).*dt.physical/xE.const  ): 'East Position'
    (   diff(yN.variable)         == (Ux.physical(1:N).*cos(Psi.physical(1:N)) - Uy.physical(1:N).*sin(Psi.physical(1:N))).*dt.physical/yN.const  ): 'North Position'
    ...
    (   diff(Ux.variable)         == (Fxf.physical.*cos(delta.physical) - Fyf.physical.*sin(delta.physical)...
                                        + Fxr.physical + m*r.physical(1:N).*Uy.physical(1:N))/m.*dt.physical/Ux.const  ): 'x Velocity'
    (   diff(Uy.variable)         == (Fxf.physical.*sin(delta.physical) + Fyf.physical.*cos(delta.physical)...
                                        + Fyr.physical - m*r.physical(1:N).*Ux.physical(1:N))/m.*dt.physical/Uy.const  ): 'y Velocity'
