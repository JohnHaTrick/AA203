function [objective, constraints, variables] = DriftNonlinearTrapezoidalFree(p)
...
% INPUT VARIABLES
delta   = mdlvar(N+1,1,'input');
% Tr      = mdlvar(N+1,15000,'input');
Fxr     = mdlvar(N+1,1e4,'input');

% LIFTING VARIABLES
alphaF  = mdlvar(N+1,1e-3);
alphaR  = mdlvar(N+1,1e-4);
Fyf     = mdlvar(N+1,1e4);
Fxf     = mdlvar(N+1,1e4);
Fzf     = mdlvar(N+1,1e4);
Fyr     = mdlvar(N+1,1e4);
...
constraints = [ constraints
    (   diff(xE.variable)         ==  Heun(-Ux.physical.*sin(Psi.physical) - Uy.physical.*cos(Psi.physical) ).*dt.physical/xE.const   ): 'East Position'
                                    
    (   diff(yN.variable)         ==  Heun( Ux.physical.*cos(Psi.physical) - Uy.physical.*sin(Psi.physical) ).*dt.physical/yN.const   ): 'North Position'
    
    (   diff(Psi.variable)        ==  Heun( r.physical ).*dt.physical/Psi.const  ): 'Orientation'
    
    (   diff(Ux.variable)         ==  Heun( (Fxf.physical.*cos(delta.physical) - Fyf.physical.*sin(delta.physical)...
                                                + Fxr.physical + m*r.physical.*Uy.physical)/m ).*dt.physical/Ux.const  ): 'x Velocity'
                                                
    (   diff(Uy.variable)         == Heun( Fxf.physical.*sin(delta.physical) + Fyf.physical.*cos(delta.physical)...
                                        + Fyr.physical - m*r.physical.*Ux.physical)/m.*dt.physical/Uy.const  ): 'y Velocity'
                                        
    (   diff(r.variable)          == Heun( (a*Fyf.physical.*cos(delta.physical) + a*Fxf.physical.*sin(delta.physical)...
...
% Slip angles
constraints = [ constraints
    tan(alphaF.physical + delta.physical).*Ux.physical/Uy.const    == (Uy.physical + a*r.physical)/Uy.const
    tan(alphaR.physical).*Ux.physical/Uy.const                     == (Uy.physical - b*r.physical)/Uy.const
    ];
...
objective = ((xE.variable(N+1) - E_f/xE.const)^2 + ...
            ...
            (r.variable(N+1) - r_f/r.const)^2)/10 + ...
             0.1*sum(diff(delta.variable).^2 + diff(Fxr.variable).^2)/N;
