...
% remove division
    tan(alphaF.physical + delta.physical).*Ux.physical(1:N)/Uy.const ... 
        == (Uy.physical(1:N) + a*r.physical(1:N))/Uy.const %+ slack.variable
    tan(alphaR.physical).*Ux.physical(1:N)/Uy.const                  ...
        == (Uy.physical(1:N) - b*r.physical(1:N))/Uy.const %+ slack.variable
% Kinematic Model:
...
objective = (Ux.variable(N)-Ux.variable(N+1))^2+(Uy.variable(N)-Uy.variable(N+1))^2+(r.variable(N)-r.variable(N+1))^2;
%             + (slack.variable).^2 ...
